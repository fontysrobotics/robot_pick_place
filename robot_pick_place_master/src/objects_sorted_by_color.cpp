		...
		if(!shapes.empty()){//must be replaced by a while statement to let the rest of the members of the vector be shown also.
			...
			for(size_t i=0; i<shapes.size(); i++){
			    ...
			    if(i<10)
			    	...
			    	oss << "base_link" << i;
				//oss << "base_link" << i;//before I have to ask if i < 10 then it should add 0 before the number.
			    odom_trans.header.frame_id = "odom";
			    odom_trans.child_frame_id = oss.str();
			
			    odom_trans.transform.translation.x = shapes[i]->getXPosition()*0.0015625;
			    odom_trans.transform.translation.y = shapes[i]->getYPosition()*0.0020833;
			    odom_trans.transform.translation.z = 0.5;//10/(shapes[i]->getArea()*0.055555);
			    ...
			    //set the position
			    odom.pose.pose.position.x = shapes[i]->getXPosition()*0.0015625;
			    odom.pose.pose.position.y = shapes[i]->getYPosition()*0.0020833;
			    odom.pose.pose.position.z = 0.5;//10/(shapes[i]->getArea()*0.055555);
