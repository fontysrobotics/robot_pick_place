...
#include <typeinfo>
#include <vector>
#include "robot_pick_place/PublishPickablePose.h" //The service that will publish the transformation (tf) of the detected object.
#include "shape.h"
#include "square.h"
#include "circle.h"
#include "triangle.h"
...
//void create_urdf(std::string frame_id){
//	for(size_t i=0; i<shapes.size(); i++){
//		shapes[i]->createModel(frame_id);
//	}
//}
     ...
     try{  
		...
		if(!shapes.empty()){//must be replaced by a while statement to let the rest of the members of the vector be shown also.
			...
			for(size_t i=0; i<shapes.size(); i++){
                ...
                ros::ServiceClient client = n.serviceClient<robot_pick_place::PublishPickablePose>("publish_pickable_pose");
                
                robot_pick_place::PublishPickablePose srv;
                
                srv.request.x_position = shapes[i]->getXPosition()*0.0015625;
                srv.request.y_position = shapes[i]->getYPosition()*0.0020833;
                srv.request.z_position = 0.5;
                srv.request.yaw_angle = ((shapes[i]->getOrientation()*M_PI)/180);
                
                if (client.call(srv)){
                    ROS_INFO("The service is successfully called");
                }
                else{
                    ROS_ERROR("Failed to call service publish_pickable_pose");
                }
			}
